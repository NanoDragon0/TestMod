...
public class ModEventHandler {
	...
	public void onEntityDamage(LivingHurtEvent event) {
		...
		ItemStack[] armor = player.inventory.armorInventory;//  -- Получение массива с слотами брони
		
		if (player.getHeldItem() != null && player.getHeldItem().getUnlocalizedName().equals("item.blood_rage") && player != null && event.entityLiving != null && player != event.entityLiving) {
		...
		if (armor[0] != null && armor[1] != null && armor[2] != null && armor[3] != null
				&& armor[3].getUnlocalizedName().equals("item.necro_helmet")
				&& armor[2].getUnlocalizedName().equals("item.necro_plate")//  -- Краш при попытке нанести сущности урон
				&& armor[1].getUnlocalizedName().equals("item.necro_legs")
				&& armor[0].getUnlocalizedName().equals("item.necro_boots")) {
			
			event.entityLiving.setHealth(event.entityLiving.getHealth() - Necro_armor.addDamage);
			((EntityPlayer) player).addChatMessage(new ChatComponentText("§6[DEBUG]: Излечено " + (float) (((event.ammount + Necro_armor.addDamage)*(Necro_armor.vampirism))) + "§4♥"));
			player.setHealth((float) (player.getHealth() + (event.ammount + Necro_armor.addDamage)*(Necro_armor.vampirism)));
		}
	}
